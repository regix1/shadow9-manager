...
class ConnectionInfo(BaseModel):
    ...
    client_address: str = Field(..., description="Client IP address")
    ...
    target_host: Optional[str] = Field(default=None, description="Target host being connected to")
    target_port: Optional[int] = Field(
        default=None, ge=1, le=65535, description="Target port being connected to"
    )
    use_tor: bool = Field(default=False, description="Whether connection uses Tor")
    connected_at: datetime = Field(
        default_factory=utc_now, description="When the connection was established"
    )
    bytes_sent: int = Field(default=0, ge=0, description="Bytes sent to target")
    bytes_received: int = Field(default=0, ge=0, description="Bytes received from target")
...
class ServerStatus(BaseModel):
    ...
    port: int = Field(..., ge=1, le=65535, description="Server bind port")
    ...
    tor_enabled: bool = Field(default=False, description="Whether Tor routing is enabled")
    uptime_seconds: Optional[float] = Field(
        default=None, ge=0, description="Server uptime in seconds"
    )
